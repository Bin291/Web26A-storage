...
- **Stack**:
  ...
  - Nơi chạy API: **Render** (free plan, Singapore — Dockerfile sẵn ở `apps/api/Dockerfile`, xem `apps/api/DEPLOY.md`)
  - Nơi chạy Web: **Cloudflare Workers Static Assets** (xem `apps/web/DEPLOY.md`)
...
|---|---|---|
| Presigned URL trực tiếp từ R2 | Trình duyệt tải thẳng từ R2, không đi qua Render | Không tốn băng thông/CPU của API instance |
...
Angular (SPA)
   ...
   ▼
NestJS API (Render) ─────────► Supabase (Postgres: metadata, roles, pgvector)
   │        ▲                        ▲
...
- File thật + AI artifacts + thumbnail + zip tạm đều nằm ở R2; Supabase chỉ lưu metadata + vector. Chiều **lên** đi qua backend, chiều **xuống** đi thẳng từ R2 bằng presigned URL (mục 5.A/5.C). Web (Angular) deploy trên **Cloudflare Workers Static Assets**, cùng hệ sinh thái với R2.
...
| Gap | Vấn đề |
|---|---|
| Hosting/deploy | NestJS đã có `apps/api/Dockerfile` build cho **Cloud Run** (nghe `$PORT`, chạy user non-root); Angular có `Dockerfile` + `nginx.conf`. Còn phải chốt: Redis production (Memorystore qua Direct VPC egress, hay Redis managed public có TLS — `REDIS_TLS=true`) và nơi host frontend. |
